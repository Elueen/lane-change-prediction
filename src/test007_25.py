...
from Models.utils import cl_labels
import pdb
...
model_re = LSTMNet(input_size=9, hidden_size=64, output_size=1, num_layers=2, sequence_size=sequence_size)
model_cl = LPLSTM(input_size=9, hidden_size=64, output_size=3, num_layers=1, sequence_size=sequence_size)
model_ln = LinearNet(input_size=4, output_size=3)

loss_fn = nn.CrossEntropyLoss()
loss_fn = loss_fn.to(device)
optimizer = torch.optim.Adam(model_ln.parameters(), lr=learning_rate)

model_paras_re = torch.load("Models/save/1919810.pth")
model_paras_cl = torch.load("Models/save/114.pth")

model_re.load_state_dict(model_paras_re)
model_cl.load_state_dict(model_paras_cl)
...
with torch.no_grad():
    ...
    total_inputs = tensor[:, :-10, :].to(device)
    total_outputs_re = model_re(total_inputs)
    total_outputs_cl = model_cl(total_inputs)

total_inputs = torch.cat((total_outputs_re, total_outputs_cl), dim=1)
tensor_dataset = TensorDataset(total_inputs, labels)
...
for epoch in range(n_epochs):
    ...
    for training_inputs, training_label in training_set:

        training_outputs = model_ln(training_inputs)
        optimizer.zero_grad()
        loss = loss_fn(training_outputs, training_label)
    ...
    with torch.no_grad():
        ...
        for val_inputs, val_labels in validating_set:
            ...
            val_outputs = model_ln(val_inputs)
...
# torch.save(model_ln.state_dict(), "Models/save/114.pth")
...
with torch.no_grad():
    ...
    for test_inputs, test_labels in testing_set:
        ...
        test_outputs = model_ln(test_inputs)
